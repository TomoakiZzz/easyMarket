import React, { Component } from 'react'
import {Link} from "react-router-dom"
import "./cateGory.scss"
class CateGory extends Component {
    render() {
        console.log(this.props)
        let {goodsData} = this.props
        return (
            <>
                <div className="cateGoryName">{goodsData.name}</div>
                <div className="cateGoryGoodsWrap">
                    {goodsData.goodsList && goodsData.goodsList.map(item=><Link to="/goods" tag='div' key={item.id}>
                        <div className="goodsItemImg">
                            <img src={item.list_pic_url}/>
                        </div>
                        <div className="goodsItemName">{item.name}</div>
                        <div className="goodsItemPrice">￥{item.retail_price}</div>

                    </Link>)}
                    <Link to="/categorys" className="categoryMoreGoods">
                        <div>更多服务好物</div>
                        <img src='data:image/png;base64,'/>
                    </Link>
                </div>
            </>
        )
    }
}
export default CateGory
